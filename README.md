...
`dist/`フォルダに`PathBrowser Setup <version>.exe`（NSISインストーラ、デスクトップ/スタートメニューへのショートカット作成込み）が生成されます。コード署名をしていないため、配布先のPCで初回起動時にSmartScreenの警告が出ます。署名証明書を取得すればこの警告は解消できますが、個人配布・ゼミ内配布の範囲では必須ではありません。
